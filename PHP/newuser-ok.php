        ...
        <div class="box">
            <div class="box-user">
                ...
                foreach ($pdo->query('select max(customer_id) from customers') as $row) {
                    $purchase_id = (int)$row['max(customer_id)'] + 1;
                }
                $sql=$pdo->prepare('insert into customers values (?,?,?,?,?,?,CURDATE(),0)');
                $sql->execute([$purchase_id,$_POST['pass'],$_POST['name'],$_POST['mail'],$_POST['postal'],$_POST['address']]);
                echo '会員登録しました。';

                $_SESSION['customer']['id'] = $purchase_id;
